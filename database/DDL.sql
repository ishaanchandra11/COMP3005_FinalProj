...
CREATE TABLE class_schedules (
    ...
    CONSTRAINT valid_schedule_capacity CHECK (current_capacity <= (
        SELECT max_capacity FROM group_classes WHERE class_id = class_schedules.class_id
    )),
...
CREATE TABLE trainer_availability (
    ...
    CONSTRAINT no_overlapping_availability EXCLUDE USING gist (
        ...
        day_of_week WITH =,
        tsrange(
            (effective_date + start_time)::timestamp,
            (COALESCE(end_date, '2099-12-31') + end_time)::timestamp
        ) WITH &&
    )
);
...
-- Function to prevent room double-booking
...
FOR EACH STATEMENT EXECUTE FUNCTION update_bill_status();
